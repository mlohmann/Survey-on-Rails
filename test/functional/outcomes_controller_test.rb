...
class OutcomesControllerTest < ActionController::TestCase
  ...
  test "should create outcome" do
    assert_difference('Outcome.count') do
      post :create, :outcome => { :description => @outcome.description, :title => @outcome.title, :token => @outcome.token }
    end
  ...
  test "should update outcome" do
    put :update, :id => @outcome, :outcome => { :description => @outcome.description, :title => @outcome.title, :token => @outcome.token }
    assert_redirected_to outcome_path(assigns(:outcome))
  end
